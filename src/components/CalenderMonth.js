...
const CalenderMonth = ( { date, days, label, timeLineData }) => {
    ...
    const occupations = timeLineData.map( ({name, interval, color }) => {
        return {
            name: name,
            interval: intervalIntersection([interval, monthInterval]),
            color: color
        }
    }).filter( ({interval}) => { return Boolean(interval); });
    ...
    return (<div className="calenderMonth" style={{width: `${monthWidth}%`, height: `.5rem`}}>
        {calenderOccupations}
    </div>);
}
